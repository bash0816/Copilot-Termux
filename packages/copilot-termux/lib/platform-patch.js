...
Module._load = function (request, parent, isMain) {
  ...
  if (typeof request === 'string' &&
    ...
    if (!isGlibcMode) {
      ...
      // lspClientOwnedProcessId は同期API(LSPサーバーのPIDを即座に取得する)であり、
      // tokio非同期のThreadsafeFunctionではない。TOKIO_PATTERNの前方一致(lspClient)で
      // 誤って一括no-op化されてしまうため、個別に除外する(実app.js 1.0.76で
      // u=h.lspClientOwnedProcessId(d) が同期的にpidとして使われることを確認済み)。
      const TOKIO_EXCLUDED = new Set(['lspClientOwnedProcessId']);
      for (const key of Object.keys(result)) {
        if (!TOKIO_EXCLUDED.has(key) && TOKIO_PATTERN.test(key) && typeof result[key] === 'function') {
